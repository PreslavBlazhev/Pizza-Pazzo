...
3. В приложението: влезте с STAFF/ADMIN акаунт и отворете админ панела —
   **само там** се появява полупрозрачното зъбно колело долу вдясно.
   Докоснете го → **Избери сдвоен принтер** → изберете принтера от списъка.
...
Зъбното колело е скрито за клиентите: показва се само докато отворената
страница е `/admin` (а дотам пуска сървърът, срещу сесийната бисквитка).
Единственото изключение е екранът „Няма връзка с интернет“ — там **задържане
върху логото** отваря настройките, за да може сгрешен начален адрес да се
поправи, когато сайтът изобщо не се зарежда.
