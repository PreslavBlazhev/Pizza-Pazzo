# Pizza Pazzo — Android приложение

Официалното приложение на пицария Pizza Pazzo. **Едно приложение за двама
потребители**: клиентът поръчва, персоналът приема поръчки и печата бележки.

- **Package:** `bg.pizzapazzo.app` · **име в системата:** Pizza Pazzo
...
## Едно приложение, два режима

```
Клиент                              Персонал (страниците /admin)
──────────────────────────────      ──────────────────────────────────────
меню · продукти · количка           живо табло · приемане/отказ
доставка/вземане · поръчка          настройки на принтера · печат
профил · история · изтриване        + нативен ESC/POS Bluetooth стек

нормално Android приложение:        терминал:
системни ленти на място             лентите се скриват
екранът заспива нормално            екранът остава буден
back излиза от приложението
```

Превключването е по URL — [`StaffRoutes`](app/src/main/java/bg/pizzapazzo/app/webview/StaffRoutes.kt).

**Това не е авторизация.** Кой какво може да види решава сървърът, от сесийната
бисквитка. Проверката по URL решава само дали екранът да остане буден и дали
принтерният bridge да отговаря — допълнителен слой, не самата защита.

**Bluetooth не се иска при стартиране.** Диалогът се появява само когато някой
отвори служебните настройки на принтера и натисне „Избери сдвоен принтер“.
Клиент, който поръчва пица, никога не го вижда.

## Какво прави

1. Отваря началната страница на сайта на цял екран, без адресна лента.
2. Инжектира `window.AndroidPrinter` bridge, чрез който служебните страници
   печатат бележки на Bluetooth термален принтер.
3. Пази login сесията (cookie), възстановява се при загуба на интернет,
   блокира навигация към чужди сайтове, отваря `tel:`/`mailto:`/външните
   линкове в съответното приложение.
...
2. **File → Open** → изберете папката `android-app` (НЕ корена на репото).
...
3. Инсталирайте. Иконата е „Pizza Pazzo“.
4. При първо стартиране се отваря началната страница на сайта. За таблото с
   поръчките влезте с STAFF/ADMIN акаунт — сесията се пази (httpOnly cookie,
   30 дни), така че следващия път се влиза направо.
...
3. В приложението: влезте с STAFF/ADMIN акаунт и отворете **Админ → Поръчки →
   На живо** (или **Админ → Настройки на печата**). Там има бутон
   **„Настройки на принтера“** → **Избери сдвоен принтер** → изберете принтера.
...
Върху страницата **не стои никакъв бутон**. Плаващото зъбно колело долу вдясно
е премахнато напълно — клиент никога не е имал работа с настройките на принтера.
Единственият вход е бутонът в админ панела, който самият сайт рисува и който
вика `AndroidPrinter.openPrinterSettings()` през моста; мостът от своя страна
отказва извикването извън служебните страници.

Едно невидимо изключение остава за аварии: на екрана „Няма връзка с интернет“
**задържане върху логото** отваря настройките. Нужно е, защото при сгрешен
начален адрес сайтът не се зарежда, админ панелът е недостижим и адресът иначе
няма как да се поправи на устройството.
...
```
app/src/main/java/bg/pizzapazzo/app/
├── MainActivity.kt              # WebView, splash, offline екран, back, режим клиент/персонал
├── PizzaPazzoApplication.kt     # споделено wiring (prefs, manager, print service)
├── webview/
│   ├── UrlParts.kt              # чист Kotlin парсер (тестваем, не android.net.Uri)
│   ├── AllowedOrigins.kt        # allowlist за навигация + bridge
│   ├── StaffRoutes.kt           # клиентска страница или служебна (по URL)
│   ├── SiteWebViewClient.kt     # навигационна политика, грешки, committed URL
│   └── JavascriptBridge.kt      # window.AndroidPrinter (+ origin и staff re-check)
├── bluetooth/
