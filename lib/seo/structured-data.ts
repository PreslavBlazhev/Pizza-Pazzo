 ...
 * Deliberately omitted until real data exists: addressLocality (the city is
 * unconfirmed), aggregateRating/review (the current reviews are sample data
 * and must never reach Google as real ones), sameAs (no social URLs on file).
 */
...
const DAYS_FOR_KEY: Record<(typeof WORKING_HOURS)[number]["dayKey"], string[]> = {
  monday: ["Monday"],
  tuesdayToThursday: ["Tuesday", "Wednesday", "Thursday"],
  friday: ["Friday"],
  saturday: ["Saturday"],
  sunday: ["Sunday"],
};
